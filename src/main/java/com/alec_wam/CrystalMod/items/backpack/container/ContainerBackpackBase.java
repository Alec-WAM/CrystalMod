...
public class ContainerBackpackBase extends Container
{
    ...
    public ContainerBackpackBase(EntityPlayer player, IItemHandler inventory)
    {
        this.player = player;
    ...
    public void addListener(IContainerListener iCrafting)
    {
        ...
        else
        {
            ...
            if(iCrafting instanceof GuiBackpackRepair || this instanceof ContainerBackpackFurnace){
    ...
    protected void syncAllSlots(EntityPlayerMP player)
    {
        for (int slot = 0; slot < this.inventorySlots.size(); slot++)
        {
            ...
            CrystalModNetwork.sendTo(new MessageSyncSlot(this.windowId, slot, stack), player);
        }
    }
    ...
    public void detectAndSendChanges()
    {
        for (int slot = 0; slot < this.inventorySlots.size(); slot++)
        {
            ...
            if (ItemStack.areItemStacksEqual(prevStack, currentStack) == false)
            {
                ...
                {
                    IContainerListener ic = (IContainerListener)this.listeners.get(j);
                    if (ic instanceof EntityPlayerMP)
                    {
                        EntityPlayerMP player = (EntityPlayerMP)ic;
                        CrystalModNetwork.sendTo(new MessageSyncSlot(this.windowId, slot, prevStack), player);
                    }
                    if(ic instanceof GuiBackpackRepair || this instanceof ContainerBackpackFurnace){
                    	((IContainerListener)this.listeners.get(j)).sendSlotContents(this, slot, prevStack);
                    }
                }
            }
        }
