...
import com.alec_wam.CrystalMod.tiles.machine.worksite.gui.elements.ITooltipRenderer;
import com.alec_wam.CrystalMod.tiles.machine.worksite.gui.elements.IWidgetSelection;
import com.alec_wam.CrystalMod.tiles.machine.worksite.gui.elements.Listener;
import com.alec_wam.CrystalMod.tiles.machine.worksite.gui.elements.Tooltip;
...
public abstract class GuiContainerWorksiteBase extends GuiContainer implements IContainerGuiCallback, ITooltipRenderer, IWidgetSelection {
	...
	public void handleElementTooltipRender(Tooltip tooltipObject) {
		...
		elementTooltipY = this.height - Mouse.getEventY() * this.height
				/ this.mc.displayHeight - 1;
	}

	protected void clearElements() {
		if (this.selectedWidget != null
				&& this.elements.contains(selectedWidget)) {
	...
	protected void sendDataToContainer(NBTTagCompound tag) {
		PacketGuiMessage pkt = new PacketGuiMessage("Data", tag);
	...
	public void handleMouseInput() throws IOException {
		...
		int type = button >= 0 ? (state ? Listener.MOUSE_DOWN
				: Listener.MOUSE_UP) : wheel != 0 ? Listener.MOUSE_WHEEL
				: Listener.MOUSE_MOVED;
		ActivationEvent evt = new ActivationEvent(type, button, state, x, y,
				wheel);
	...
	public void handleKeyboardInput() throws IOException {
		...
		ActivationEvent evt = new ActivationEvent(state ? Listener.KEY_DOWN
				: Listener.KEY_UP, key, ch, state);
		for (GuiElement element : this.elements) {
	...
	protected void keyTyped(char ch, int key) throws IOException {
		if (selectedWidget == null) {
			boolean isExitCommand = key == 1
					|| key == this.mc.gameSettings.keyBindInventory
							.getKeyCode();
			if (isExitCommand) {
	...
	/**
	 * over-rideable method for onGuiClosed w/ proper functionality -- (is
	 * called before the container disappears) derived classes can override this
	 * to handle custom onClosed functionality (e.g. call container to send data
	 * to server/ finalize settings)
	 * 
	 * @return true if the GUI should closed -- false if another GUI has been
	 *         opened/the GUI should not closed
	 */
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		if (elementTooltip != null) {
			elementTooltip.renderTooltip(elementTooltipX, elementTooltipY,
					partialRenderTick);
			elementTooltip = null;
		}
	}
	...
	/**
	 * call this method to enforce a re-initialization of gui at the start of
	 * next game tick (not render tick)<br>
	 ...
	 * Only called a single time, shortly after construction, but before any
	 * rendering or other update methods are called
	 */
	public abstract void initElements();

	/**
	 * sub-classes should implement this method to setup/change any elements
	 * that need adjusting when the gui is initialized<br>
	 * any elements that are positioned outside of the gui-window space will
	 * need their positions updated by calling element.setPosition(xPos, yPos)
	 * as they reference internal position relative to the guiLeft / guiTop
	 * values from this gui (which are passed in and updated directly after
	 * setupElements() is called)<br>
	 * Always called at least once, directly after {@link #initElements()}
	 */
	public abstract void setupElements();

	/**
	 * sub-classes should override this method to handle any expected packet
	 * data
	 */
	@Override
	...
	/**
	 * deferred to allow proper render-order, and draw the tooltip on top of
	 * everything else
	 */
	@Override
	...
	/**
	 * Action event for gui-widgets. A single event is sent to all element /
	 * listeners for each input event (key up/down, mouse up/down/move/wheel)
	 * 
	 * @author Shadowmage
	 */
	public class ActivationEvent {
		...
		/**
		 * the mouse button number. -1 = none 0 = RMB 1 = LMB 2+= ?
		 */
		public int mButton;
		/**
		...
		private ActivationEvent(int type, int button, boolean state, int mx,
				int my, int mw) {
			this.type = type;
			this.mButton = button;
			this.state = state;
	...
	public static void popViewport() {
		...
		if (p != null) {
			...
			GL11.glScissor(p.x * guiScale, mc.displayHeight - p.y * guiScale
					- p.h * guiScale, p.w * guiScale, p.h * guiScale);
		} else {
			GL11.glDisable(GL11.GL_SCISSOR_TEST);
		}
	}
	...
	/**
	 * class used to represent a currently drawable portion of the screen. Used
	 * in a stack for figuring out what composites may draw where
	 * 
	 * @author John
	 * 
	 */
