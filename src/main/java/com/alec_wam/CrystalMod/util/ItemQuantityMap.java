...
public class ItemQuantityMap {
	...
	/**
	 * puts all key/value pairs from the incoming map. Overwrites existing
	 * counts.
	 * 
	 * @param incoming
	 */
	...
	/**
	 * is not a PUT operation -- merges quantities (values) instead of
	 * overwriting
	 * 
	 * @param incoming
	 */
	...
	/**
	 * removes all key/value pairs that have corresponding keys contained in
	 * toRemove
	 * 
	 * @param toRemove
	 */
	...
	/**
	 * @param entries
	 *            must not be the exact key-set or
	 ...
	 * if toRemove contains a key that this. does not, it is silently ignored,
	 * and removal continues at the next key
	 * 
	 * @param toRemove
	 */
	...
	/**
	 * Return the most compact set of item-stacks that can represent the
	 * contents of this map.<br>
	 * May return multiple stacks of the same item if the quantity contained is
	 * > maxStackSize.<br>
	 * *
	 * 
	 * @param items
	 *            will be filled with the item-stacks from this map, must not be
	 *            NULL
	 */
	 ...
	 * Stack max size for a given item is ignored -- stack size may be >64 (and
	 * may actually be up to Integer.MAX_VALUE)
	 * 
	 * @param items
	 *            will be filled with the item-stacks from this map, must not be
	 *            NULL
	...
	/**
	 * return the total number of inventory slots this quantity map would use if
	 * placed into slotted inventory
	 * 
	 * @return
	 */
	...
	/**
	 * used by ItemQuantityMap for tracking item quantities for a given
	 * ItemStackHashWrap
	 * 
	 * @author Shadowmage
	 * 
	...
	/**
	 * Lightweight wrapper for an item stack as a hashable object suitable for
	 * use as keys in maps.<br>
	...
	public static final class ItemHashEntry {
		...
		public int hashCode() {
			...
			hash = 31 * hash + damage;
			// hash = 31*hash + stack.stackSize;//noop for implementation, only
			// care about identity, not quantity
			if (itemTag != null) {
		...
		public static NBTTagCompound writeToNBT(ItemHashEntry entry, NBTTagCompound tag) {
			tag.setString("itemName", Item.REGISTRY.getNameForObject(entry.item).toString());
