...
public class ContainerBlockHolder extends Container implements IMessageHandler 
{
    public ItemStack blockHolder;

    public ContainerBlockHolder(IInventory playerInventory, ItemStack blockHolder)
    {
    		    	...
    		    	if(clickTypeIn == ClickType.QUICK_MOVE && (dragType == 0 || dragType == 1)){
    		            ...
    		            if(ItemStackTools.isValid(blockStack)){
    		            	if(ItemUtil.canCombine(blockStack, slotStack)){
    		            		int added = ItemBlockHolder.addBlocks(blockHolder, ItemStackTools.getStackSize(slotStack));
    		            		if(added > 0){
	...
	public void handleMessage(String messageId, NBTTagCompound messageData, boolean client) {
		...
		if(messageId.equalsIgnoreCase("ShiftTransfer")){
			ItemStack blockStack = ItemBlockHolder.getBlockStack(blockHolder);
			if(ItemStackTools.isValid(blockStack)){
				int size = Math.min(64, ItemBlockHolder.getBlockCount(blockHolder));
				...
				if(ItemStackTools.getStackSize(copy) < size){
					...
					ItemBlockHolder.removeBlocks(blockHolder, removed);
