...
public class EntityDagger extends EntityArrow implements IEntityAdditionalSpawnData {
	...
	public EntityDagger(World world, EntityPlayer player, float speed, float inaccuracy) {
		this(world);
	...
	public boolean dealDamage(float speed, EntityLivingBase attacker, Entity target) {
		return target.attackEntityFrom(new DamageSourceDarkarang("dagger", attacker, attacker), 8.0F);
	}
	...
	public boolean playerPickup(EntityPlayer player, boolean blocked){
		...
		if(!blocked){
			if(player.inventory.addItemStackToInventory(new ItemStack(ModItems.dagger))){
				return true;
			}
		}
