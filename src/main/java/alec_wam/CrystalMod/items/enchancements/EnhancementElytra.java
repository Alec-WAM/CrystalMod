...
public class EnhancementElytra implements IEnhancement {

	@Override
	public ResourceLocation getID(){
		return CrystalMod.resourceL("elytra");
	}
	...
	public String getNBTID() {
		return CrystalMod.prefix("elytra");
	}

	@Override
	public NonNullList<ItemStack> getRequiredItems() {
		return NonNullList.withSize(1, new ItemStack(Items.ELYTRA));
	}
	...
	public boolean removeItemsFromPlayer(EntityPlayer player) {
		...
		search : for(int i = 0; i < player.inventory.getSizeInventory(); i++){
			...
			if(ItemStackTools.isValid(stack) && stack.getItem() == Items.ELYTRA){
		...
		if(remaining > 0 && removed > 0){
			player.inventory.addItemStackToInventory(new ItemStack(Items.ELYTRA, removed));
		}
	...
	public boolean returnItemsToPlayer(EntityPlayer player) {
		final ItemStack returnStack = new ItemStack(Items.ELYTRA);
	...
	public void addToolTip(ItemStack stack, List<String> list) {
		list.add(TextFormatting.DARK_PURPLE+""+TextFormatting.UNDERLINE+ItemUtil.getLocalizedName(Items.ELYTRA));
	}
