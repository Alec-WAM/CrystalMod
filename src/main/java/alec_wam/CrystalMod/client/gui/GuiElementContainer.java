 ...
 * @author King Lemming
 */
public abstract class GuiElementContainer<C extends Container> extends ContainerScreen<C> {
	...
	public GuiElementContainer(C container, PlayerInventory inventory, ITextComponent title) {

		super(container, inventory, title);
	}

	public GuiElementContainer(C container, PlayerInventory inventory, ITextComponent title, ResourceLocation texture) {
	...
	protected void drawGuiContainerForegroundLayer(int x, int y) {
		...
		if (drawTitle) {
			font.drawString(Lang.translateToLocal(name), getCenteredOffset(Lang.translateToLocal(name)), 6, 0x404040);
		}
		if (drawInventory) {
			font.drawString(Lang.translateToLocal("container.inventory"), 8, ySize - 96 + 3, 0x404040);
		}
		drawElements(0, true);
	}
	...
	public Slot getSlotAtPosition(int xCoord, int yCoord) {
		...
		for (int k = 0; k < getContainer().inventorySlots.size(); ++k) {
			Slot slot = getContainer().inventorySlots.get(k);
	...
	public List<String> handleTooltip(int mousex, int mousey, List<String> tooltip) {
		if (ItemStackTools.isEmpty(minecraft.player.inventory.getItemStack())) {
			addTooltips(tooltip);
		}
		return tooltip;
	}
	...
	public void drawTooltip(List<String> list) {
		this.renderTooltip(list, mouseX + guiLeft, mouseY + guiTop);
		tooltip.clear();
	}
	...
	protected int getCenteredOffset(String string, int xWidth) {
		...
		return (xWidth - font.getStringWidth(string)) / 2;
	}
