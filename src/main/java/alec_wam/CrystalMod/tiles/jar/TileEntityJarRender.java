...
public class TileEntityJarRender extends TileEntityRenderer<TileEntityJar> {
	...
	public static void renderInternalJar(double x, double y, double z, PotionType potion, int potionCount, boolean shulker, EnumMap<EnumFacing, Boolean> label, int pass){
		if(shulker){
			...
			if(bullet == null)bullet = new EntityShulkerBullet(Minecraft.getInstance().world);
	...
	public static void renderPotionLabel(PotionEffect effect, double x, double y, double z, EnumFacing side){
		...
		if(effect !=null){
	        ...
	        Minecraft.getInstance().getTextureManager().bindTexture(GuiContainer.INVENTORY_BACKGROUND);
	        if (potion.hasStatusIcon())
	        {
	            int i1 = potion.getStatusIconIndex();
	            double textureX = i1 % 12 * 18;
	            double textureY = 198 + i1 / 12 * 18;
			...
			if(!potion.isInstant()){
        		String time = PotionUtil.getPotionDurationString(effect, 1.0F);
        		GlStateManager.pushMatrix();
