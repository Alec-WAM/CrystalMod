...
public class TileEntityFusionPedestal extends TileEntityInventory implements IMessageHandler, IFusionPedestal {
	...
	public void tick(){
		...
		BlockState below = getWorld().getBlockState(under);
		...
		if(!getWorld().isRemote){
			if(this.shouldDoWorkThisTick(5)){
				if(isCrafting.getValue() !=isCrafting.getLastValue()){
					...
					nbt.putBoolean("Value", isCrafting.getValue());
			...
			if(this.shouldDoWorkThisTick(2)){
				if(craftingProgress.getValue() !=craftingProgress.getLastValue()){
					...
					CompoundNBT nbt = new CompoundNBT();
					nbt.putInt("Value", craftingProgress.getValue());
				...
				if(craftingCooldown.getValue() !=craftingCooldown.getLastValue()){
					...
					CompoundNBT nbt = new CompoundNBT();
					nbt.putInt("Value", craftingCooldown.getValue());
	...
	public void startCrafting(@Nullable PlayerEntity player){
		pedestalSearch();
	...
	public void pedestalSearch(){
		...
		if(facing == Direction.UP){
			searchList = Lists.newArrayList(BlockPos.getAllInBoxMutable(getPos().add(-range, 0, -range), getPos().add(range, 2, range)));
		} 
		else if(facing == Direction.DOWN){
			searchList = Lists.newArrayList(BlockPos.getAllInBoxMutable(getPos().add(-range, -2, -range), getPos().add(range, 0, range)));
		} 
		else if(facing == Direction.NORTH){
			searchList = Lists.newArrayList(BlockPos.getAllInBoxMutable(getPos().add(-range, -range, -2), getPos().add(range, range, 0)));
		} 
		else if(facing == Direction.SOUTH){
			searchList = Lists.newArrayList(BlockPos.getAllInBoxMutable(getPos().add(-range, -range, 0), getPos().add(range, range, 2)));
		} 
		else if(facing == Direction.WEST){
			searchList = Lists.newArrayList(BlockPos.getAllInBoxMutable(getPos().add(-2, -range, -range), getPos().add(0, range, range)));
		} 
		else if(facing == Direction.EAST){
			searchList = Lists.newArrayList(BlockPos.getAllInBoxMutable(getPos().add(0, -range, -range), getPos().add(2, range, range)));
		} 
		else {
			searchList = Lists.newArrayList(BlockPos.getAllInBoxMutable(getPos().add(-range, 0, -range), getPos().add(range, 0, range)));
		}
		...
		for(BlockPos searchPos : searchList){
			if(getWorld().isBlockLoaded(searchPos)){
				...
				if(tile instanceof TileEntityPedestal){
					...
					Direction dir = Direction.getFacingFromVector(facingPos.getX(), facingPos.getY(), facingPos.getZ());
