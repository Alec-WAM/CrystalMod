...
public class TileEntityMetalCrystalChest extends TileEntityInventory implements INamedContainerProvider
{
    private int ticksSinceSync = -1;
    public float prevLidAngle;
    public float lidAngle;
    private int numUsingPlayers;
    public MetalCrystalChestType type;

    public TileEntityMetalCrystalChest()
    {
    	this(MetalCrystalChestType.BLUE);
    }

    public TileEntityMetalCrystalChest(MetalCrystalChestType type)
    {
        super(ModBlocks.metalChestGroup.getTileType(type), "", type.size);
        this.type = type;
    }
    ...
    public void readCustomNBT(CompoundNBT nbt){
    	...
    	type = MetalCrystalChestType.values()[nbt.getInt("Type")];    	
    }
    ...
    public void tick()
    {
        ...
        if (hasWorld() && !this.getWorld().isRemote && this.numUsingPlayers != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (var4.openContainer instanceof ContainerMetalCrystalChest)
                {
        ...
        {
            getWorld().addBlockEvent(pos, ModBlocks.metalChestGroup.getBlock(type), 1, numUsingPlayers);
        }
	...
	public Container createMenu(int i, PlayerInventory playerInventory, PlayerEntity playerIn) {
		return new ContainerMetalCrystalChest(i, playerInventory, this, type, 0, 0);
	}
