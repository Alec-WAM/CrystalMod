...
public class PipeNetworkEnergy extends PipeNetworkBase<TileEntityPipeEnergy> {
	...
	public boolean addPipe(TileEntityPipeBase p) {
		...
		if(super.addPipe(p)){
			...
			Set<Direction> externalDirs = pipe.getExternalConnections();
			for (Direction dir : externalDirs) {
	...
	public void powerNodeAdded(TileEntityPipeEnergy energyPipe, Direction direction, IEnergyNode energyNode) {
		if(energyNode == null) {
			return;
		}
