...
public class ContainerPipeFilter extends Container
{
    ...
    public ContainerPipeFilter(int windowId, PlayerEntity player, ItemStack stack, Hand hand)
    {
    	super(null, windowId);
    	this.filter = stack;
    	this.hand = hand;
    ...
    protected void addPlayerInventory(PlayerInventory paramPlayerInventory)
    {
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 9; j++)
            {
                this.addSlot(new Slot(paramPlayerInventory, j + i * 9 + 9, 8 + j * 18, 62 + i * 18));
            }
        }
        for (int i = 0; i < 9; i++)
            this.addSlot(new Slot(paramPlayerInventory, i, 8 + i * 18, 120));
    }
    ...
    public void onContainerClosed(PlayerEntity player){
        ...
        if(ItemStackTools.isValid(held)){
        	...
        	ItemNBTHelper.getCompound(held).put("FilterItems", inv);
        }
	}
    
    @Override
	public ItemStack slotClick(int slotId, int clickedButton, ClickType mode, PlayerEntity player)
	{
