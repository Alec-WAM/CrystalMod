...
public class NetworkInventory {
	...
	NetworkInventory(PipeNetworkItem network, TileEntityPipeItem pip, Direction pipDir, NetworkPos location) {
	...
	public boolean hasTarget(TileEntityPipeItem pipe, Direction dir) {
	...
	private void calculateDistances(List<Target> targets, Map<NetworkPos, Integer> visited, List<NetworkPos> steps, int distance) {
		...
		for (NetworkPos np : steps) {
			...
			if(con != null) {
				for (Direction dir : con.getExternalConnections()) {
				...
				for (Direction dir : con.getPipeConnections()) {
	...
	private Target getTarget(List<Target> targets, TileEntityPipeItem con, Direction dir) {
	...
	public void setInventorySide(Direction inventorySide) {
